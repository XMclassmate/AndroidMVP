...
public class TextViewUtil {
    ...
    private static volatile TextViewUtil mTextViewUtil;

    public synchronized static TextViewUtil getInstance() {
        if (mTextViewUtil == null) {
            synchronized (GsonUtils.class) {
                if (mTextViewUtil == null) {
                    mTextViewUtil = new TextViewUtil();
                }
            }
        }
        return mTextViewUtil;
    }
    ...
    public SpannableString getSizeSpanSpToPx(Context context, String str, int start, int end, int spSize) {
        SpannableString ss = new SpannableString(str);
        ss.setSpan(new AbsoluteSizeSpan(CommonUtil.sp2px(context, spSize)), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        return ss;
    }
